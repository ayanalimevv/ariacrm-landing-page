...
import { ChatContent } from './hero-dashboard/chat-content';
import { DataModelContent } from './hero-dashboard/data-model-content';
import { ReportingContent } from './hero-dashboard/reporting-content';
import { DashboardSidebar } from './hero-dashboard/sidebar';
import { WorkflowsContent } from './hero-dashboard/workflows-content';

/* -------------------------------------------------------------------------- */
/*  Tabs                                                                      */
/* -------------------------------------------------------------------------- */

type TabId = 'ask' | 'reporting' | 'workflows' | 'data-model';
...
const TAB_DURATIONS: Record<TabId, number> = {
  ask: 16000,
  reporting: 8000,
  workflows: 14000,
  'data-model': 8000,
...
const CONTENT_TITLES: Record<TabId, string> = {
  ask: 'Q3 revenue review',
  reporting: 'Q3 revenue review',
  'data-model': 'Q3 revenue review',
...
/* -------------------------------------------------------------------------- */
/*  Workspace Header — copied verbatim from hero-dashboard/index.tsx          */
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
/*  Content Header — copied verbatim from hero-dashboard/index.tsx           */
/* -------------------------------------------------------------------------- */

function ContentHeader({ activeTab }: { activeTab: TabId }) {
  const isSlack = activeTab === 'workflows';
...
function TabContentForId({ tabId }: { tabId: TabId }) {
  if (tabId === 'ask') return <ChatContent />;
  if (tabId === 'reporting') return <ReportingContent />;
  if (tabId === 'workflows') return <WorkflowsContent />;
  if (tabId === 'data-model') return <DataModelContent />;
  return null;
}

/* -------------------------------------------------------------------------- */
/*  DashboardCard — mirrors the hero's DashboardCard verbatim                 */
/* -------------------------------------------------------------------------- */
...
const DashboardCard = memo(function DashboardCard({
  ...
  return (
    <div className='overflow-hidden rounded-lg border border-border bg-card shadow-[0px_2px_6px_0px_rgba(28,40,64,0.06),0px_6px_20px_-2px_rgba(28,40,64,0.08)] sm:rounded-xl dark:shadow-[0px_2px_6px_0px_rgba(0,0,0,0.4),0px_6px_20px_-2px_rgba(0,0,0,0.5)]'>
      <div className='relative grid h-[340px] w-full grid-rows-[auto_1fr] sm:h-[420px] md:h-[500px] lg:h-[600px] lg:grid-cols-[272px_1fr] lg:grid-rows-[auto_1fr]'>
        <WorkspaceHeader />
        <ContentHeader activeTab={tabId} />
        <DashboardSidebar />
...
/* -------------------------------------------------------------------------- */
/*  Main FeatureTabs — outer card-stack flip animation                        */
/* -------------------------------------------------------------------------- */

export default function FeatureTabs() {
  const [activeId, setActiveId] = useState<TabId>('ask');
  const [progress, setProgress] = useState(0);
  const [isActive, setIsActive] = useState(true);
  const activeIdRef = useRef<TabId>('ask');
  const rootRef = useRef<HTMLDivElement>(null);

  activeIdRef.current = activeId;

  // Pause cycling when offscreen or browser tab hidden
  useEffect(() => {
    if (typeof window === 'undefined') return;

    let inView = true;
    let docVisible = !document.hidden;
    const update = () => setIsActive(inView && docVisible);
  ...
  // Auto-advance progress and switch tabs
  useEffect(() => {
    if (!isActive) return;
    const interval = setInterval(() => {
      setProgress((prev) => {
        const duration = TAB_DURATIONS[activeIdRef.current] ?? 8000;
        const next = prev + 100 / (duration / 50);
        if (next >= 100) {
          const idx = TABS.findIndex((t) => t.id === activeIdRef.current);
          const nextIdx = (idx + 1) % TABS.length;
          const nextTab = TABS[nextIdx]?.id ?? TABS[0]?.id ?? 'ask';
          setActiveId(nextTab);
          return 0;
        }
        return next;
  ...
  // Reorder so active tab's card is at the front of the stack
  const orderedTabs = useMemo(() => {
    const i = TABS.findIndex((t) => t.id === activeId);
    return [...TABS.slice(i), ...TABS.slice(0, i)];
  }, [activeId]);
  ...
  return (
    <section ref={rootRef} className='py-20 md:py-28'>
      <div className='mx-auto max-w-7xl px-6 lg:px-8'>
        ...
        {/* Tab triggers with per-tab progress fill */}
        <div className='mt-14 grid grid-cols-1 gap-x-4 gap-y-2 border-border border-b sm:grid-cols-2 lg:grid-cols-4'>
          {TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'group relative flex flex-col px-2 pt-3 pb-4 text-left transition-opacity',
                  isActiveTab ? 'opacity-100' : 'opacity-60 hover:opacity-100',
                )}
              >
                ...
                {/* Progress fill */}
                <span className='-bottom-px absolute right-0 left-0 h-0.5 overflow-hidden'>
                  {isActiveTab && (
                    <span
        ...
        {/* Card stack — each card is locked to one tab id; reorder brings active to front */}
        <div className='relative mx-auto mt-14 h-[360px] sm:h-[440px] md:h-[520px] lg:h-[640px]'>
          {orderedTabs.map((tab, i) => (
            <motion.div
              ...
              animate={{
                top: i * -10,
                scale: 1 - i * 0.04,
                zIndex: orderedTabs.length - i,
                opacity: i > 2 ? 0 : 1,
              }}
              transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
            >
              <DashboardCard tabId={tab.id} />
            </motion.div>
          ))}
        </div>
